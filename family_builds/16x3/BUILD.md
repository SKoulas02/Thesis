# Build: 16x3  (16 cores x 3 blocks = 48 C Blocks)
...
| DSPs | **384** |
| MACs / cycle | 96 |
| HBM pseudo-channels | **13** of 32 (19 spare) |
| -- weights | 6 (1536 bits) |
| -- indices | 2 (512 bits, 480 used) |
| -- activations | 2 (512 bits -- always, the window is 32 elements) |
| -- output | 3 (768 bits) |
| W_IDX | 6 (= 2 x BLOCKS_NUM) |
| OOC Fmax (bare engine, 1.5 ns) | 501.8 MHz |
| **in-system target** | **350 MHz** (0.70 x OOC) |
...
350 MHz is 0.697 x this config's OOC number, i.e. at or below that
...
|---|---|---|
| `Design/two2N_axis.vhd` | stays here; `gen_xo` reads it from here | AXIS splitter, 13 interfaces, generics defaulted to this config |
| `Design/krnl_gemv_sparse.v` | stays here | RTL kernel top, 13 stream ports |
| `gen_xo_16x3.tcl` | `~/` on the server | packages `krnl_gemv_sparse_16x3.xo` |
| `sparse_hbm_16x3.cfg` | `~/Vitis/` | nk / sp= / stream_connect |
| `slr_floorplan_16x3.cfg` | `~/Vitis/` | gemv->SLR1, movers->SLR0 |
| `host_defs_16x3.h` | reference | the 6 constants for `host_sparse.cpp` |
...
**1. Copy up.** SERVER LAYOUT, confirmed 2026-09-10 -- one directory per build:
    ...
    ~/GEMV_Sparse/Vitis_16x3/    <- NEW, this build
...
```bash
# on the server
cd ~/GEMV_Sparse && mkdir -p Vitis_16x3
cp Vitis/krnl_mm2s.hw.xo Vitis/krnl_s2mm.hw.xo \
   Vitis/krnl_mm2s.hw_emu.xo Vitis/krnl_s2mm.hw_emu.xo Vitis_16x3/
df -h /home
```

```bash
# from the local repo root
scp -r family_builds/16x3 skoulas@coroni:/home/skoulas/GEMV_Sparse/family_builds/
scp family_builds/16x3/gen_xo_16x3.tcl skoulas@coroni:/home/skoulas/
scp family_builds/16x3/sparse_hbm_16x3.cfg \
    family_builds/16x3/slr_floorplan_16x3.cfg \
    Vitis/impl_family.cfg skoulas@coroni:/home/skoulas/GEMV_Sparse/Vitis_16x3/
```
...
```tcl
source /home/skoulas/gen_xo_16x3.tcl
```

Produces `/home/skoulas/GEMV_Sparse/krnl_gemv_sparse_16x3.xo`. **Verify by
listing the archive, never by size** -- a shell-only .xo links happily and
computes nothing:

```bash
unzip -l ~/GEMV_Sparse/krnl_gemv_sparse_16x3.xo | grep -cE "\.vhd|\.xci"
```

Want **>= 18** (12 VHDL + 7 .xci, minus whatever the packager renames).

**3. hw_emu first.** From `~/GEMV_Sparse/Vitis_16x3`. This config's channel topology (13 streams) has never been
...
v++ -t hw_emu --platform $PLATFORM --config sparse_hbm_16x3.cfg \
    --kernel_frequency 350 -l -o sparse_16x3.hw_emu.xclbin \
...
**4. Link for hardware.** In `tmux`, from `~/GEMV_Sparse/Vitis_16x3`:

```bash
v++ -t hw --platform $PLATFORM \
    --config sparse_hbm_16x3.cfg \
    --config impl_family.cfg \
    --config slr_floorplan_16x3.cfg \
    --kernel_frequency 350 \
    -l -o sparse_16x3_350.xclbin \
...
**5. Host.** Set the six constants from `host_defs_16x3.h` at the top of
`host_sparse.cpp` and rebuild. Nothing else in that file changes -- it was
written against these constants throughout.

**6. Stimulus.** Set `W_PCS=6 IND_PCS=2 A_PCS=2 CORES=16
BLOCKS=3 SPARSITY_BIT=480` in `gen_timing_stimulus.py` and
...
- the link reports **WNS >= 0** at 350 MHz
- `xclbinutil --info` lists **13** HBM channels bound
