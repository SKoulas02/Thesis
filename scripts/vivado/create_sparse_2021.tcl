# ----------------------------------------------------------------------------
# create_sparse_2021.tcl  --  builds the SPARSE 2:M GEMV project on Vivado 2021.1
#
...
# those .xci files, and the dense script uses the identical block, so both
...
#   <root>/scripts/vivado/create_sparse_2021.tcl
#   <root>/GEMV_4.0_Source/{Design,Simulation,Emulation}, timing.xdc
#
# USAGE (from the repository root)
#   vivado -mode batch -source scripts/vivado/create_sparse_2021.tcl -nojournal -nolog
#   vivado -mode batch -source scripts/vivado/create_sparse_2021.tcl -tclargs impl   ;# + synth/impl
#
# Add "impl" to also run OOC synthesis + implementation and write reports.
# Note: synthesis MUST be out-of-context -- the top has 4402 ports, far beyond
# any package, so a bonded build cannot place.
# ----------------------------------------------------------------------------

# ---- configuration ---------------------------------------------------------
set root      [file normalize [file join [file dirname [info script]] .. ..]]
set src       "$root/GEMV_4.0_Source"
set proj_name "GEMV_4.0"
set proj_dir  "$root/$proj_name"
set part      "xcu280-fsvh2892-2L-e"
set jobs      8

# Run synthesis + implementation as well?
#   batch : vivado -mode batch -source scripts/vivado/create_sparse_2021.tcl -tclargs impl
...
# ---- design sources --------------------------------------------------------
# The sparse datapath: c_block (with the 2 x 32:1 index gather), c_core,
# weights_fifo_2k (8 weight PCs + 3 index PCs), the two2N top, plus the FP and
# FIFO wrappers shared byte-identically with the dense project.
...
# ============================================================================
# IP  --  identical block in create_dense_2021.tcl. Keep them in sync.
# ============================================================================
...
# backpressures the read engine natively). axis_data_fifo_pc serves BOTH the 8
# weight PCs and the 3 index PCs -- the index instances tie their tlast off.
...
# NOTE: two2N_TB.vhd hard-codes WINDOWS absolute paths for weights.hex /
# indices.hex / activations.hex / output.txt / tlast.txt. Simulation on this
# server needs those constants edited to Linux paths first. Synthesis and
# implementation are unaffected.
add_files -fileset sim_1 -norecurse "$src/Simulation/two2N_TB.vhd"
add_files -fileset constrs_1 -norecurse "$src/timing.xdc"

set_property top two2N    [get_filesets sources_1]
set_property top two2N_TB [get_filesets sim_1]
set_property top_lib xil_defaultlib [get_filesets sim_1]
set_property -name {xsim.simulate.runtime} -value {-all} -objects [get_filesets sim_1]

# ---- OOC synthesis ---------------------------------------------------------
# Mandatory: two2N has 4402 top-level ports (2048 weights + 768 indices + 512
...
#
# The 774-port index bus is a genuine cost of sparsity support -- report it as
# HBM pseudo-channels (17 vs the dense 14), not buried in the LUT count.
...
puts "=== synth top = two2N   sim top = two2N_TB   (OOC)"
...
puts " any numbers -- and confirm these are IDENTICAL to what the"
puts " dense script reported, or the area comparison is invalid:"
puts "=============================================================="
...
puts " and make sure the DENSE project reports the same values."
puts "=============================================================="
