// GENERATED by make_family_build.py for 8x4 (8 cores x 4 blocks).
//
...
static const int W_PCS     = 4;
static const int IND_PCS   = 2;
static const int A_PCS     = 2;
static const int C_PCS     = 2;
static const int LANES     = 32;   // 8 cores x 4 blocks
...
static const int IND_BITS  = 320;   // 10 x LANES, index bits in the joined word
static const int SP_PC     = IND_BITS / 256;         // = 1, which index PC
static const int SP_BYTE   = (IND_BITS % 256) / 8;   // = 8, byte within it
...
// the configuration. For 8x4 (IND_BITS = 320) it is:
//
//     return (unsigned)(ind_pc1.p[k * PC_BYTES + 8] & 0x3);
//
// i.e. index PC 1, byte 8.
//
...
//   W_PCS=4  IND_PCS=2  A_PCS=2  C_PCS=2
//   CORES=8  BLOCKS=4  LANES=32  SPARSITY_BIT=320  (bit of the joined word)
