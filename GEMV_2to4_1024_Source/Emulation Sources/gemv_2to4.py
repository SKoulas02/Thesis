...
# ---- Configuration ---------------------------------------------------------
A_FILE      = Path(__file__).parent / "A_1024x512_reordered.txt"
IND_FILE    = Path(__file__).parent / "indices_1024x256_reordered.txt"
B_FILE      = Path(__file__).parent / "B_1024x1_reordered.txt"
OUTPUT_FILE = Path(__file__).parent / "C_1024x1.txt"

M           = 1024   # output vector length / matrix rows
K_DENSE     = 512    # kept (non-zero) cols per row
K_LOGICAL   = 1024   # logical x length (must equal 2 * K_DENSE for 2:4)
