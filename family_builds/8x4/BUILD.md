# Build: 8x4  (8 cores x 4 blocks = 32 C Blocks)
...
| DSPs | **256** |
| MACs / cycle | 64 |
| HBM pseudo-channels | **10** of 32 (22 spare) |
| -- weights | 4 (1024 bits) |
| -- indices | 2 (512 bits, 320 used) |
| -- activations | 2 (512 bits -- always, the window is 32 elements) |
| -- output | 2 (512 bits) |
| W_IDX | 8 (= 2 x BLOCKS_NUM) |
| OOC Fmax (bare engine, 1.5 ns) | 559.9 MHz |
| **in-system target** | **375 MHz** (0.67 x OOC) |
...
375 MHz is 0.670 x this config's OOC number, i.e. at or below that
...
|---|---|---|
| `Design/two2N_axis.vhd` | stays here; `gen_xo` reads it from here | AXIS splitter, 10 interfaces, generics defaulted to this config |
| `Design/krnl_gemv_sparse.v` | stays here | RTL kernel top, 10 stream ports |
| `gen_xo_8x4.tcl` | `~/` on the server | packages `krnl_gemv_sparse_8x4.xo` |
| `sparse_hbm_8x4.cfg` | `~/Vitis/` | nk / sp= / stream_connect |
| `slr_floorplan_8x4.cfg` | `~/Vitis/` | gemv->SLR1, movers->SLR0 |
| `host_defs_8x4.h` | reference | the 6 constants for `host_sparse.cpp` |
...
**1. Copy up.** SERVER LAYOUT, confirmed 2026-09-10 -- one directory per build:
    ...
    ~/GEMV_Sparse/Vitis_8x4/    <- NEW, this build
...
```bash
# on the server
cd ~/GEMV_Sparse && mkdir -p Vitis_8x4
cp Vitis/krnl_mm2s.hw.xo Vitis/krnl_s2mm.hw.xo \
   Vitis/krnl_mm2s.hw_emu.xo Vitis/krnl_s2mm.hw_emu.xo Vitis_8x4/
df -h /home
```

```bash
# from the local repo root
scp -r family_builds/8x4 skoulas@coroni:/home/skoulas/GEMV_Sparse/family_builds/
scp family_builds/8x4/gen_xo_8x4.tcl skoulas@coroni:/home/skoulas/
scp family_builds/8x4/sparse_hbm_8x4.cfg \
    family_builds/8x4/slr_floorplan_8x4.cfg \
    Vitis/impl_family.cfg skoulas@coroni:/home/skoulas/GEMV_Sparse/Vitis_8x4/
```
...
```tcl
source /home/skoulas/gen_xo_8x4.tcl
```

Produces `/home/skoulas/GEMV_Sparse/krnl_gemv_sparse_8x4.xo`. **Verify by
listing the archive, never by size** -- a shell-only .xo links happily and
computes nothing:

```bash
unzip -l ~/GEMV_Sparse/krnl_gemv_sparse_8x4.xo | grep -cE "\.vhd|\.xci"
```

Want **>= 18** (12 VHDL + 7 .xci, minus whatever the packager renames).

**3. hw_emu first.** From `~/GEMV_Sparse/Vitis_8x4`. This config's channel topology (10 streams) has never been
...
v++ -t hw_emu --platform $PLATFORM --config sparse_hbm_8x4.cfg \
    --kernel_frequency 375 -l -o sparse_8x4.hw_emu.xclbin \
...
**4. Link for hardware.** In `tmux`, from `~/GEMV_Sparse/Vitis_8x4`:

```bash
v++ -t hw --platform $PLATFORM \
    --config sparse_hbm_8x4.cfg \
    --config impl_family.cfg \
    --config slr_floorplan_8x4.cfg \
    --kernel_frequency 375 \
    -l -o sparse_8x4_375.xclbin \
...
**5. Host.** Set the six constants from `host_defs_8x4.h` at the top of
`host_sparse.cpp` and rebuild. Nothing else in that file changes -- it was
written against these constants throughout.

**6. Stimulus.** Set `W_PCS=4 IND_PCS=2 A_PCS=2 CORES=8
BLOCKS=4 SPARSITY_BIT=320` in `gen_timing_stimulus.py` and
...
- the link reports **WNS >= 0** at 375 MHz
- `xclbinutil --info` lists **10** HBM channels bound
