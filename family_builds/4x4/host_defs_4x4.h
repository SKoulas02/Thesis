// GENERATED by make_family_build.py for 4x4 (4 cores x 4 blocks).
//
...
static const int W_PCS     = 2;
static const int IND_PCS   = 1;
static const int A_PCS     = 2;
static const int C_PCS     = 1;
static const int LANES     = 16;   // 4 cores x 4 blocks
...
static const int IND_BITS  = 160;   // 10 x LANES, index bits in the joined word
static const int SP_PC     = IND_BITS / 256;         // = 0, which index PC
static const int SP_BYTE   = (IND_BITS % 256) / 8;   // = 20, byte within it
...
// the configuration. For 4x4 (IND_BITS = 160) it is:
//
//     return (unsigned)(ind_pc0.p[k * PC_BYTES + 20] & 0x3);
//
// i.e. index PC 0, byte 20.
//
...
//   W_PCS=2  IND_PCS=1  A_PCS=2  C_PCS=1
//   CORES=4  BLOCKS=4  LANES=16  SPARSITY_BIT=160  (bit of the joined word)
