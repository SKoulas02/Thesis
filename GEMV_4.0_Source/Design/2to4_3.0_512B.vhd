...
entity two2four is
    generic(
        ...
        A_ROWS      : integer := 64     -- Number of Rows of Matrix A
    );
    port(
        clk         : in std_logic;
        resetn      : in std_logic;
...
architecture two2four_arch of two2four is
    ...
    component matrix_fifo_512 is
        ...
        port(
            ...
            indices     : in std_logic_vector (BUS_EL*EL_SIZE-1 downto 0);     -- Test for RR Logic
            ind_valid   : in std_logic;                          
    ...
    signal adder_input      : std_logic_vector ((EL_SIZE*BUS_EL/B_IDX)-1 downto 0) := (others => '0');
    signal adder_valid      : std_logic := '0';
    signal adder_tlast      : std_logic := '0';

    signal adder_output     : std_logic_vector (EL_SIZE-1 downto 0) := (others => '0');
    signal adder_valid_out  : std_logic := '0';
    signal adder_tlast_out  : std_logic := '0';
...
begin
    ...
    begin
        if rising_edge(clk) then
            if resetn = '0' then
                ...
                if adder_valid_out = '1' then

                    Cout <= adder_output;
                    Cvalid <= '1';
                    if counter_output = A_ROWS-1 then
                        Ctlast <= '1';
                        counter_output <= 0;
                    else
                        Ctlast <= '0';
                ...
                if c_tlast_internal(counter_adder) = '1' then
                    ...
                    adder_input <= c_internal(counter_adder);
                    adder_valid <= '1';
    ...
    VECTOR_FIFO_INSTANCE : vector_fifo_512
    generic map(
        EL_SIZE     => EL_SIZE,
        BUS_EL      => BUS_EL
    )
    ...
    MATRIX_FIFO_INSTANCE : matrix_fifo_512
    generic map(
        EL_SIZE     => EL_SIZE,
        BUS_EL      => BUS_EL,
        A_IDX       => A_IDX,
    ...
    port map(
        ...
        in_elements     => adder_input,     -- *** INTERNAL SIGNAL FROM C ROWS ***
        in_valid        => adder_valid,
        tlast           => adder_tlast,

        out_elements    => adder_output,              -- Final Output
        out_valid       => adder_valid_out,
        tlast_out       => adder_tlast_out
    );
