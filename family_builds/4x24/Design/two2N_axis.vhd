...
--
-- GENERATED by make_family_build.py for the 4x24 configuration
-- (4 cores x 24 blocks = 96 blocks, 768 DSPs, 24 HBM pseudo-channels).
...
-- two2N declares its streams BUNDLED -- s_axis_w_tdata is one 3072-bit port with
-- a 12-bit tvalid/tready/tlast vector, and so on. The Vitis RTL Kernel Wizard
-- needs 24 SEPARATE, properly named AXIS interfaces so the packager can infer
...
-- So the wrapper DECLARES the 4 index tlast input(s) and leaves them
...
entity two2N_axis is
    generic(
        ...
        W_IDX       : integer := 48;    -- matrix elements per core = 2 x BLOCKS_NUM
        A_IDX       : integer := 32;    -- vector elements (the window)
        IND_NUM     : integer := 10;    -- index bits per block
        BLOCKS_NUM  : integer := 24;    -- C blocks per core (2 elements each)
        CORES_NUM   : integer := 4;    -- C Cores

        PC_WIDTH    : integer := 256;   -- HBM pseudo-channel data width
        W_PCS       : integer := 12;    -- weight PCs (12*256 = 3072 bits)
        A_PCS       : integer := 2;    -- activation PCs
        IND_PCS     : integer := 4;    -- index PCs (4*256 = 1024, 960 used)
        C_PCS       : integer := 6;    -- output PCs (6*256 = 1536 bits)
        IND_BITS    : integer := 960    -- useful index bits (remainder is padding)
    );
    port(
        ap_clk   : in std_logic;
        ap_rst_n : in std_logic;

        -- ---- Weights : 12 slave AXIS channels ----
        ...
        -- ---- Indices : 4 slave AXIS channel(s) ----
        ...
        -- ---- Output : 6 master AXIS channel(s) ----
...
begin
    ...
    --    [961:960] of the joined index word -- it is buffer CONTENT written by
    ...
    -- =======================================================================
    -- 3. SPLIT the bundled output bus into the 6 master channel(s), and drive
