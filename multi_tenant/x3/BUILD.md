# 3 x 4x4 multi-tenant build

3 independent tenants, 18 of 32 HBM channels, 18 mover CUs + 3 engines = **21 compute units**, target **400 MHz**.

Floorplan: **slr_floorplan_4x4_x3.cfg** (every CU in SLR0). Everything in SLR0 would take ~375 of 672 BRAM (56%).
...
```bash
# from the repo root
scp multi_tenant/x3/*.cfg multi_tenant/host_sparse_multi.cpp \
    multi_tenant/prep_tenants.py multi_tenant/run_multi_measure.py \
    skoulas@coroni.microlab.ntua.gr:/home/skoulas/GEMV_Sparse/Vitis_multi_x3/
```
...
```bash
mkdir -p ~/GEMV_Sparse/Vitis_multi_x3 && cd ~/GEMV_Sparse/Vitis_multi_x3
...
v++ -t hw_emu --platform $PLATFORM --config sparse_hbm_4x4_x3.cfg \
    --kernel_frequency 400 -l -o sparse_4x4_x3.hw_emu.xclbin \
...
./host_sparse_multi sparse_4x4_x3.hw_emu.xclbin 400 0 0:t0 1:t1 2:t2
...
v++ -t hw --platform $PLATFORM \
    --config sparse_hbm_4x4_x3.cfg \
    --config impl_family.cfg \
    --config slr_floorplan_4x4_x3.cfg \
    --kernel_frequency 400 \
    -l -o sparse_4x4_x3_400.xclbin \
...
If it lands far below the target, try `slr_floorplan_4x4_x3_split.cfg`; never change the
strategy.
...
mkdir -p ~/GEMV_Sparse/reports_multi_x3 && cp -r _x/reports/link/imp ~/GEMV_Sparse/reports_multi_x3/
```
...
./host_sparse_multi sparse_4x4_x3_400.xclbin 400 0 0:t0 1:t1 2:t2
# each tenant ALONE -- the uncontended baseline for interference
./host_sparse_multi sparse_4x4_x3_400.xclbin 400 0 0:t0
./host_sparse_multi sparse_4x4_x3_400.xclbin 400 0 1:t1
./host_sparse_multi sparse_4x4_x3_400.xclbin 400 0 2:t2
```
...
- `xclbinutil --info` lists **18** HBM channels bound and **21** CUs
- the link reports **WNS >= 0** at 400 MHz (and note what it closed at)
