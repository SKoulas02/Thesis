...
-- (8 cores x 8 blocks = 64 blocks, 512 DSPs, 17 HBM pseudo-channels).
...
-- a 8-bit tvalid/tready/tlast vector, and so on. The Vitis RTL Kernel Wizard
-- needs 17 SEPARATE, properly named AXIS interfaces so the packager can infer
...
-- So the wrapper DECLARES the 3 index tlast input(s) and leaves them
...
entity two2N_axis is
    generic(
        ...
        W_IDX       : integer := 16;    -- matrix elements per core = 2 x BLOCKS_NUM
        A_IDX       : integer := 32;    -- vector elements (the window)
        IND_NUM     : integer := 10;    -- index bits per block
        BLOCKS_NUM  : integer := 8;    -- C blocks per core (2 elements each)
        CORES_NUM   : integer := 8;    -- C Cores
        ...
        C_PCS       : integer := 4;    -- output PCs (4*256 = 1024 bits)
    ...
    port(
        ...
        -- ---- Indices : 3 slave AXIS channel(s) ----
        ...
        -- ---- Output : 4 master AXIS channel(s) ----
...
begin
    ...
    -- =======================================================================
    -- 3. SPLIT the bundled output bus into the 4 master channel(s), and drive
