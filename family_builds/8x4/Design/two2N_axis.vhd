...
--
-- GENERATED by make_family_build.py for the 8x4 configuration
-- (8 cores x 4 blocks = 32 blocks, 256 DSPs, 10 HBM pseudo-channels).
...
-- two2N declares its streams BUNDLED -- s_axis_w_tdata is one 1024-bit port with
-- a 4-bit tvalid/tready/tlast vector, and so on. The Vitis RTL Kernel Wizard
-- needs 10 SEPARATE, properly named AXIS interfaces so the packager can infer
...
entity two2N_axis is
    generic(
        ...
        W_PCS       : integer := 4;    -- weight PCs (4*256 = 1024 bits)
        A_PCS       : integer := 2;    -- activation PCs
        IND_PCS     : integer := 2;    -- index PCs (2*256 = 512, 320 used)
        C_PCS       : integer := 2;    -- output PCs (2*256 = 512 bits)
        IND_BITS    : integer := 320    -- useful index bits (remainder is padding)
    );
    port(
        ap_clk   : in std_logic;
        ap_rst_n : in std_logic;

        -- ---- Weights : 4 slave AXIS channels ----
        ...
        -- ---- Output : 2 master AXIS channel(s) ----
...
begin
    ...
    --    [321:320] of the joined index word -- it is buffer CONTENT written by
    ...
    -- =======================================================================
    -- 3. SPLIT the bundled output bus into the 2 master channel(s), and drive
