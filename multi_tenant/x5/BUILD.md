# 5 x 4x4 multi-tenant build

5 independent tenants, 30 of 32 HBM channels, 30 mover CUs + 5 engines = **35 compute units**, target **375 MHz**.
...
```bash
# from the repo root
scp multi_tenant/x5/*.cfg multi_tenant/host_sparse_multi.cpp \
    multi_tenant/prep_tenants.py \
    skoulas@coroni.microlab.ntua.gr:/home/skoulas/GEMV_Sparse/Vitis_multi_x5/
```
...
```bash
mkdir -p ~/GEMV_Sparse/Vitis_multi_x5 && cd ~/GEMV_Sparse/Vitis_multi_x5
...
```bash
cd ~/GEMV_Sparse/Vitis_multi_x5
python3 prep_tenants.py --emu ~/GEMV_Sparse/GEMV_4.0_Source/Emulation \
                        --out . --tenants 5
```

Writes `t0/ .. t4/`, each with `bin/`, its own `golden.txt` and its own copy
...
v++ -t hw_emu --platform $PLATFORM --config sparse_hbm_4x4_x5.cfg \
    --kernel_frequency 375 -l -o sparse_4x4_x5.hw_emu.xclbin \
    ../krnl_gemv_sparse_4x4.xo krnl_mm2s.hw_emu.xo krnl_s2mm.hw_emu.xo
```
...
g++ -Wall -O2 -std=c++1y -I$XILINX_XRT/include host_sparse_multi.cpp \
    -L$XILINX_XRT/lib -lOpenCL -lrt -lstdc++ -pthread -o host_sparse_multi
./host_sparse_multi sparse_4x4_x5.hw_emu.xclbin 375 0 0:t0 1:t1 2:t2 3:t3 4:t4
for k in $(seq 0 4); do (cd t$k && python3 compare_gemv4_py36.py); done
...
v++ -t hw --platform $PLATFORM \
    --config sparse_hbm_4x4_x5.cfg \
    --config impl_family.cfg \
    --config slr_floorplan_4x4_x5.cfg \
    --kernel_frequency 375 \
    -l -o sparse_4x4_x5_375.xclbin \
    ../krnl_gemv_sparse_4x4.xo krnl_mm2s.hw.xo krnl_s2mm.hw.xo
```

Same frozen strategy as all six family builds -- do not vary it. If this
misses, lower `--kernel_frequency`, or try
`slr_floorplan_4x4_x5_split.cfg`; never change the strategy.
...
mkdir -p ~/GEMV_Sparse/reports_multi_x5 && cp -r _x/reports/link/imp ~/GEMV_Sparse/reports_multi_x5/
```
...
./host_sparse_multi sparse_4x4_x5_375.xclbin 375 0 0:t0 1:t1 2:t2 3:t3 4:t4
# each tenant ALONE -- the uncontended baseline for interference
./host_sparse_multi sparse_4x4_x5_375.xclbin 375 0 0:t0
./host_sparse_multi sparse_4x4_x5_375.xclbin 375 0 1:t1
./host_sparse_multi sparse_4x4_x5_375.xclbin 375 0 2:t2
./host_sparse_multi sparse_4x4_x5_375.xclbin 375 0 3:t3
./host_sparse_multi sparse_4x4_x5_375.xclbin 375 0 4:t4
```
...
- `xclbinutil --info` lists **30** HBM channels bound and **35** CUs
