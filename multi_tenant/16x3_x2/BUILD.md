# 2 x 16x3 multi-tenant build

2 independent tenants, 26 of 32 HBM channels, 26 mover CUs + 2 engines = **28 compute units**, target **375 MHz**.

Floorplan: **slr_floorplan_16x3_x2_split.cfg** (engines in SLR1, movers in SLR0 beside the HBM). Everything in SLR0 would take ~514 of 672 BRAM (76%).
...
```bash
# from the repo root
scp multi_tenant/16x3_x2/*.cfg multi_tenant/host_sparse_multi.cpp \
    multi_tenant/prep_tenants.py multi_tenant/run_multi_measure.py \
    skoulas@coroni.microlab.ntua.gr:/home/skoulas/GEMV_Sparse/Vitis_multi_16x3_x2/
```
...
```bash
mkdir -p ~/GEMV_Sparse/Vitis_multi_16x3_x2 && cd ~/GEMV_Sparse/Vitis_multi_16x3_x2
...
```bash
cd ~/GEMV_Sparse/Vitis_multi_16x3_x2
...
v++ -t hw_emu --platform $PLATFORM --config sparse_hbm_16x3_x2.cfg \
    --kernel_frequency 375 -l -o sparse_16x3_x2.hw_emu.xclbin \
...
./host_sparse_multi sparse_16x3_x2.hw_emu.xclbin 375 0 0:t0 1:t1
...
v++ -t hw --platform $PLATFORM \
    --config sparse_hbm_16x3_x2.cfg \
    --config impl_family.cfg \
    --config slr_floorplan_16x3_x2_split.cfg \
    --kernel_frequency 375 \
    -l -o sparse_16x3_x2_375.xclbin \
...
If it lands far below the target, try `slr_floorplan_16x3_x2.cfg`; never change the
strategy.
...
mkdir -p ~/GEMV_Sparse/reports_multi_16x3_x2 && cp -r _x/reports/link/imp ~/GEMV_Sparse/reports_multi_16x3_x2/
```
...
./host_sparse_multi sparse_16x3_x2_375.xclbin 375 0 0:t0 1:t1
# each tenant ALONE -- the uncontended baseline for interference
./host_sparse_multi sparse_16x3_x2_375.xclbin 375 0 0:t0
./host_sparse_multi sparse_16x3_x2_375.xclbin 375 0 1:t1
```
...
- `xclbinutil --info` lists **26** HBM channels bound and **28** CUs
