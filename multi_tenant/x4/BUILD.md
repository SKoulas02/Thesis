# 4 x 4x4 multi-tenant build

4 independent tenants, 24 of 32 HBM channels, 24 mover CUs + 4 engines = **28 compute units**, target **375 MHz**.

Floorplan: **slr_floorplan_4x4_x4.cfg** (every CU in SLR0). Everything in SLR0 would take ~490 of 672 BRAM (73%).
...
```bash
# from the repo root
scp multi_tenant/x4/*.cfg multi_tenant/host_sparse_multi.cpp \
    multi_tenant/prep_tenants.py multi_tenant/run_multi_measure.py \
    skoulas@coroni.microlab.ntua.gr:/home/skoulas/GEMV_Sparse/Vitis_multi_x4/
```
...
```bash
mkdir -p ~/GEMV_Sparse/Vitis_multi_x4 && cd ~/GEMV_Sparse/Vitis_multi_x4
...
```bash
cd ~/GEMV_Sparse/Vitis_multi_x4
python3 prep_tenants.py --emu ~/GEMV_Sparse/GEMV_4.0_Source/Emulation \
                        --out . --tenants 4
```

Writes `t0/ .. t3/`, each with `bin/`, its own `golden.txt` and its own copy
...
v++ -t hw_emu --platform $PLATFORM --config sparse_hbm_4x4_x4.cfg \
    --kernel_frequency 375 -l -o sparse_4x4_x4.hw_emu.xclbin \
...
./host_sparse_multi sparse_4x4_x4.hw_emu.xclbin 375 0 0:t0 1:t1 2:t2 3:t3
for k in $(seq 0 3); do (cd t$k && python3 compare_gemv4_py36.py); done
...
v++ -t hw --platform $PLATFORM \
    --config sparse_hbm_4x4_x4.cfg \
    --config impl_family.cfg \
    --config slr_floorplan_4x4_x4.cfg \
    --kernel_frequency 375 \
    -l -o sparse_4x4_x4_375.xclbin \
...
If it lands far below the target, try `slr_floorplan_4x4_x4_split.cfg`; never change the
strategy.
...
mkdir -p ~/GEMV_Sparse/reports_multi_x4 && cp -r _x/reports/link/imp ~/GEMV_Sparse/reports_multi_x4/
```
...
./host_sparse_multi sparse_4x4_x4_375.xclbin 375 0 0:t0 1:t1 2:t2 3:t3
# each tenant ALONE -- the uncontended baseline for interference
./host_sparse_multi sparse_4x4_x4_375.xclbin 375 0 0:t0
./host_sparse_multi sparse_4x4_x4_375.xclbin 375 0 1:t1
./host_sparse_multi sparse_4x4_x4_375.xclbin 375 0 2:t2
./host_sparse_multi sparse_4x4_x4_375.xclbin 375 0 3:t3
```
