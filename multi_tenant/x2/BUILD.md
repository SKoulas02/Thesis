# 2 x 4x4 multi-tenant build

2 independent tenants, 12 of 32 HBM channels, 12 mover CUs + 2 engines = **14 compute units**, target **400 MHz**.
...
```bash
# from the repo root
scp multi_tenant/x2/*.cfg multi_tenant/host_sparse_multi.cpp \
    multi_tenant/prep_tenants.py \
    skoulas@coroni.microlab.ntua.gr:/home/skoulas/GEMV_Sparse/Vitis_multi_x2/
```
...
```bash
mkdir -p ~/GEMV_Sparse/Vitis_multi_x2 && cd ~/GEMV_Sparse/Vitis_multi_x2
...
v++ -t hw_emu --platform $PLATFORM --config sparse_hbm_4x4_x2.cfg \
    --kernel_frequency 400 -l -o sparse_4x4_x2.hw_emu.xclbin \
...
./host_sparse_multi sparse_4x4_x2.hw_emu.xclbin 400 0 0:t0 1:t1
...
v++ -t hw --platform $PLATFORM \
    --config sparse_hbm_4x4_x2.cfg \
    --config impl_family.cfg \
    --config slr_floorplan_4x4_x2.cfg \
    --kernel_frequency 400 \
    -l -o sparse_4x4_x2_400.xclbin \
...
`slr_floorplan_4x4_x2_split.cfg`; never change the strategy.
...
mkdir -p ~/GEMV_Sparse/reports_multi_x2 && cp -r _x/reports/link/imp ~/GEMV_Sparse/reports_multi_x2/
```
...
./host_sparse_multi sparse_4x4_x2_400.xclbin 400 0 0:t0
...
- `xclbinutil --info` lists **12** HBM channels bound and **14** CUs
