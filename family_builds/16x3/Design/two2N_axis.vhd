...
--
-- GENERATED by make_family_build.py for the 16x3 configuration
-- (16 cores x 3 blocks = 48 blocks, 384 DSPs, 13 HBM pseudo-channels).
...
-- two2N declares its streams BUNDLED -- s_axis_w_tdata is one 1536-bit port with
-- a 6-bit tvalid/tready/tlast vector, and so on. The Vitis RTL Kernel Wizard
-- needs 13 SEPARATE, properly named AXIS interfaces so the packager can infer
...
-- So the wrapper DECLARES the 2 index tlast input(s) and leaves them
...
entity two2N_axis is
    generic(
        ...
        W_IDX       : integer := 6;    -- matrix elements per core = 2 x BLOCKS_NUM
        A_IDX       : integer := 32;    -- vector elements (the window)
        IND_NUM     : integer := 10;    -- index bits per block
        BLOCKS_NUM  : integer := 3;    -- C blocks per core (2 elements each)
        CORES_NUM   : integer := 16;    -- C Cores

        PC_WIDTH    : integer := 256;   -- HBM pseudo-channel data width
        W_PCS       : integer := 6;    -- weight PCs (6*256 = 1536 bits)
        A_PCS       : integer := 2;    -- activation PCs
        IND_PCS     : integer := 2;    -- index PCs (2*256 = 512, 480 used)
        C_PCS       : integer := 3;    -- output PCs (3*256 = 768 bits)
        IND_BITS    : integer := 480    -- useful index bits (remainder is padding)
    );
    port(
        ap_clk   : in std_logic;
        ap_rst_n : in std_logic;

        -- ---- Weights : 6 slave AXIS channels ----
        ...
        -- ---- Indices : 2 slave AXIS channel(s) ----
        ...
        -- ---- Output : 3 master AXIS channel(s) ----
...
begin
    ...
    --    [481:480] of the joined index word -- it is buffer CONTENT written by
    ...
    -- =======================================================================
    -- 3. SPLIT the bundled output bus into the 3 master channel(s), and drive
