// GENERATED by make_family_build.py for 16x3 (16 cores x 3 blocks).
//
...
static const int W_PCS     = 6;
static const int IND_PCS   = 2;
static const int A_PCS     = 2;
static const int C_PCS     = 3;
static const int LANES     = 48;   // 16 cores x 3 blocks
...
static const int IND_BITS  = 480;   // 10 x LANES, index bits in the joined word
static const int SP_PC     = IND_BITS / 256;         // = 1, which index PC
static const int SP_BYTE   = (IND_BITS % 256) / 8;   // = 28, byte within it
...
// the configuration. For 16x3 (IND_BITS = 480) it is:
//
//     return (unsigned)(ind_pc1.p[k * PC_BYTES + 28] & 0x3);
//
// i.e. index PC 1, byte 28.
//
...
//   W_PCS=6  IND_PCS=2  A_PCS=2  C_PCS=3
//   CORES=16  BLOCKS=3  LANES=48  SPARSITY_BIT=480  (bit of the joined word)
