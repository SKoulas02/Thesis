# 2 x 8x4 multi-tenant build

2 independent tenants, 20 of 32 HBM channels, 20 mover CUs + 2 engines = **22 compute units**, target **375 MHz**.

Floorplan: **slr_floorplan_8x4_x2.cfg** (every CU in SLR0). Everything in SLR0 would take ~400 of 672 BRAM (60%).
...
```bash
# from the repo root
scp multi_tenant/8x4_x2/*.cfg multi_tenant/host_sparse_multi.cpp \
    multi_tenant/prep_tenants.py multi_tenant/run_multi_measure.py \
    skoulas@coroni.microlab.ntua.gr:/home/skoulas/GEMV_Sparse/Vitis_multi_8x4_x2/
```
...
```bash
mkdir -p ~/GEMV_Sparse/Vitis_multi_8x4_x2 && cd ~/GEMV_Sparse/Vitis_multi_8x4_x2
...
python3 prep_tenants.py --emu ~/GEMV_Sparse/GEMV_4.0_Source/Emulation \
                        --out . --tenants 2 --cores 8 --blocks 4
```
...
v++ -t hw_emu --platform $PLATFORM --config sparse_hbm_8x4_x2.cfg \
    --kernel_frequency 375 -l -o sparse_8x4_x2.hw_emu.xclbin \
...
./host_sparse_multi sparse_8x4_x2.hw_emu.xclbin 375 0 0:t0 1:t1
...
v++ -t hw --platform $PLATFORM \
    --config sparse_hbm_8x4_x2.cfg \
    --config impl_family.cfg \
    --config slr_floorplan_8x4_x2.cfg \
    --kernel_frequency 375 \
    -l -o sparse_8x4_x2_375.xclbin \
...
If it lands far below the target, try `slr_floorplan_8x4_x2_split.cfg`; never change the
strategy.
...
mkdir -p ~/GEMV_Sparse/reports_multi_8x4_x2 && cp -r _x/reports/link/imp ~/GEMV_Sparse/reports_multi_8x4_x2/
```
...
./host_sparse_multi sparse_8x4_x2_375.xclbin 375 0 0:t0 1:t1
# each tenant ALONE -- the uncontended baseline for interference
./host_sparse_multi sparse_8x4_x2_375.xclbin 375 0 0:t0
./host_sparse_multi sparse_8x4_x2_375.xclbin 375 0 1:t1
```
...
- `xclbinutil --info` lists **20** HBM channels bound and **22** CUs
