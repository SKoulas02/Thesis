// GENERATED by make_family_build.py for 4x24 (4 cores x 24 blocks).
//
...
static const int W_PCS     = 12;
static const int IND_PCS   = 4;
static const int A_PCS     = 2;
static const int C_PCS     = 6;
static const int LANES     = 96;   // 4 cores x 24 blocks
...
static const int IND_BITS  = 960;   // 10 x LANES, index bits in the joined word
static const int SP_PC     = IND_BITS / 256;         // = 3, which index PC
static const int SP_BYTE   = (IND_BITS % 256) / 8;   // = 24, byte within it
...
// the configuration. For 4x24 (IND_BITS = 960) it is:
//
//     return (unsigned)(ind_pc3.p[k * PC_BYTES + 24] & 0x3);
//
// i.e. index PC 3, byte 24.
//
...
//   W_PCS=12  IND_PCS=4  A_PCS=2  C_PCS=6
//   CORES=4  BLOCKS=24  LANES=96  SPARSITY_BIT=960  (bit of the joined word)
