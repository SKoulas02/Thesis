# Build: 8x8  (8 cores x 8 blocks = 64 C Blocks)
...
| DSPs | **512** |
| MACs / cycle | 128 |
| HBM pseudo-channels | **17** of 32 (15 spare) |
| -- weights | 8 (2048 bits) |
| -- indices | 3 (768 bits, 640 used) |
| -- activations | 2 (512 bits -- always, the window is 32 elements) |
| -- output | 4 (1024 bits) |
| W_IDX | 16 (= 2 x BLOCKS_NUM) |
| OOC Fmax (bare engine, 1.5 ns) | 448.6 MHz |
| **in-system target** | **325 MHz** (0.72 x OOC) |
...
325 MHz is 0.724 x this config's OOC number, i.e. at or below that
...
|---|---|---|
| `Design/two2N_axis.vhd` | stays here; `gen_xo` reads it from here | AXIS splitter, 17 interfaces, generics defaulted to this config |
| `Design/krnl_gemv_sparse.v` | stays here | RTL kernel top, 17 stream ports |
| `gen_xo_8x8.tcl` | `~/` on the server | packages `krnl_gemv_sparse_8x8.xo` |
| `sparse_hbm_8x8.cfg` | `~/Vitis/` | nk / sp= / stream_connect |
| `slr_floorplan_8x8.cfg` | `~/Vitis/` | gemv->SLR1, movers->SLR0 |
| `host_defs_8x8.h` | reference | the 6 constants for `host_sparse.cpp` |
...
**1. Copy up.** SERVER LAYOUT, confirmed 2026-09-10 -- one directory per build:
    ...
    ~/GEMV_Sparse/Vitis_8x8/    <- NEW, this build
...
```bash
# on the server
cd ~/GEMV_Sparse && mkdir -p Vitis_8x8
cp Vitis/krnl_mm2s.hw.xo Vitis/krnl_s2mm.hw.xo \
   Vitis/krnl_mm2s.hw_emu.xo Vitis/krnl_s2mm.hw_emu.xo Vitis_8x8/
df -h /home
```

```bash
# from the local repo root
scp -r family_builds/8x8 skoulas@coroni:/home/skoulas/GEMV_Sparse/family_builds/
scp family_builds/8x8/gen_xo_8x8.tcl skoulas@coroni:/home/skoulas/
scp family_builds/8x8/sparse_hbm_8x8.cfg \
    family_builds/8x8/slr_floorplan_8x8.cfg \
    Vitis/impl_family.cfg skoulas@coroni:/home/skoulas/GEMV_Sparse/Vitis_8x8/
```
...
```tcl
source /home/skoulas/gen_xo_8x8.tcl
```

Produces `/home/skoulas/GEMV_Sparse/krnl_gemv_sparse_8x8.xo`. **Verify by
listing the archive, never by size** -- a shell-only .xo links happily and
computes nothing:

```bash
unzip -l ~/GEMV_Sparse/krnl_gemv_sparse_8x8.xo | grep -cE "\.vhd|\.xci"
```

Want **>= 18** (12 VHDL + 7 .xci, minus whatever the packager renames).

**3. hw_emu first.** From `~/GEMV_Sparse/Vitis_8x8`. This config's channel topology (17 streams) has never been
...
v++ -t hw_emu --platform $PLATFORM --config sparse_hbm_8x8.cfg \
    --kernel_frequency 325 -l -o sparse_8x8.hw_emu.xclbin \
    ../krnl_gemv_sparse_8x8.xo krnl_mm2s.hw_emu.xo krnl_s2mm.hw_emu.xo
```
...
**4. Link for hardware.** In `tmux`, from `~/GEMV_Sparse/Vitis_8x8`:

```bash
v++ -t hw --platform $PLATFORM \
    --config sparse_hbm_8x8.cfg \
    --config impl_family.cfg \
    --config slr_floorplan_8x8.cfg \
    --kernel_frequency 325 \
    -l -o sparse_8x8_325.xclbin \
    ../krnl_gemv_sparse_8x8.xo krnl_mm2s.hw.xo krnl_s2mm.hw.xo
```
...
**5. Host.** Set the six constants from `host_defs_8x8.h` at the top of
`host_sparse.cpp` and rebuild. Nothing else in that file changes -- it was
written against these constants throughout.

**6. Stimulus.** Set `W_PCS=8 IND_PCS=3 A_PCS=2 CORES=8
BLOCKS=8 SPARSITY_BIT=640` in `gen_timing_stimulus.py` and
...
- the link reports **WNS >= 0** at 325 MHz
- `xclbinutil --info` lists **17** HBM channels bound
