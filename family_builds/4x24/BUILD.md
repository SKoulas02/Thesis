# Build: 4x24  (4 cores x 24 blocks = 96 C Blocks)
...
| DSPs | **768** |
| MACs / cycle | 192 |
| HBM pseudo-channels | **24** of 32 (8 spare) |
| -- weights | 12 (3072 bits) |
| -- indices | 4 (1024 bits, 960 used) |
| -- activations | 2 (512 bits -- always, the window is 32 elements) |
| -- output | 6 (1536 bits) |
| W_IDX | 48 (= 2 x BLOCKS_NUM) |
| OOC Fmax (bare engine, 1.5 ns) | 407.3 MHz |
| **in-system target** | **300 MHz** (0.74 x OOC) |
...
300 MHz is 0.737 x this config's OOC number, i.e. at or below that
...
|---|---|---|
| `Design/two2N_axis.vhd` | stays here; `gen_xo` reads it from here | AXIS splitter, 24 interfaces, generics defaulted to this config |
| `Design/krnl_gemv_sparse.v` | stays here | RTL kernel top, 24 stream ports |
| `gen_xo_4x24.tcl` | `~/` on the server | packages `krnl_gemv_sparse_4x24.xo` |
| `sparse_hbm_4x24.cfg` | `~/Vitis/` | nk / sp= / stream_connect |
| `slr_floorplan_4x24.cfg` | `~/Vitis/` | gemv->SLR1, movers->SLR0 |
| `host_defs_4x24.h` | reference | the 6 constants for `host_sparse.cpp` |
...
**1. Copy up.** SERVER LAYOUT, confirmed 2026-09-10 -- one directory per build:
    ...
    ~/GEMV_Sparse/Vitis_4x24/    <- NEW, this build
...
```bash
# on the server
cd ~/GEMV_Sparse && mkdir -p Vitis_4x24
cp Vitis/krnl_mm2s.hw.xo Vitis/krnl_s2mm.hw.xo \
   Vitis/krnl_mm2s.hw_emu.xo Vitis/krnl_s2mm.hw_emu.xo Vitis_4x24/
df -h /home
```

```bash
# from the local repo root
scp -r family_builds/4x24 skoulas@coroni:/home/skoulas/GEMV_Sparse/family_builds/
...
```tcl
source /home/skoulas/gen_xo_4x24.tcl
```

Produces `/home/skoulas/GEMV_Sparse/krnl_gemv_sparse_4x24.xo`. **Verify by
listing the archive, never by size** -- a shell-only .xo links happily and
computes nothing:

```bash
unzip -l ~/GEMV_Sparse/krnl_gemv_sparse_4x24.xo | grep -cE "\.vhd|\.xci"
```

Want **>= 18** (12 VHDL + 7 .xci, minus whatever the packager renames).

**3. hw_emu first.** From `~/GEMV_Sparse/Vitis_4x24`. This config's channel topology (24 streams) has never been
...
v++ -t hw_emu --platform $PLATFORM --config sparse_hbm_4x24.cfg \
    --kernel_frequency 300 -l -o sparse_4x24.hw_emu.xclbin \
    ../krnl_gemv_sparse_4x24.xo krnl_mm2s.hw_emu.xo krnl_s2mm.hw_emu.xo
```
...
**4. Link for hardware.** In `tmux`, from `~/GEMV_Sparse/Vitis_4x24`:

```bash
v++ -t hw --platform $PLATFORM \
    --config sparse_hbm_4x24.cfg \
    --config impl_family.cfg \
    --config slr_floorplan_4x24.cfg \
    --kernel_frequency 300 \
    -l -o sparse_4x24_300.xclbin \
    ../krnl_gemv_sparse_4x24.xo krnl_mm2s.hw.xo krnl_s2mm.hw.xo
```
...
**5. Host.** Set the six constants from `host_defs_4x24.h` at the top of
`host_sparse.cpp` and rebuild. Nothing else in that file changes -- it was
written against these constants throughout.

**6. Stimulus.** Set `W_PCS=12 IND_PCS=4 A_PCS=2 CORES=4
BLOCKS=24 SPARSITY_BIT=960` in `gen_timing_stimulus.py` and
...
- the link reports **WNS >= 0** at 300 MHz
- `xclbinutil --info` lists **24** HBM channels bound
